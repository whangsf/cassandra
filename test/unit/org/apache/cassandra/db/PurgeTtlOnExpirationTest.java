...
public class PurgeTtlOnExpirationTest
{
    ...
    public void testPurgeTtlOnExpiration() throws Exception
    {
        ...
        MigrationManager.announceTableUpdate(cfs.metadata().unbuild().purgeTtlOnExpiration(Boolean.FALSE).build(), true);
        String key = "ttl";
        long timestamp = FBUtilities.nowInSeconds();
        ColumnMetadata colDef = cfs.metadata().getColumn(new ColumnIdentifier("col1", true));
        ...
        cfs.truncateBlocking();
        MigrationManager.announceTableUpdate(cfs.metadata().unbuild().purgeTtlOnExpiration(Boolean.TRUE).build(), true);
        cfs.reload();
        new RowUpdateBuilder(cfs.metadata(), timestamp, ttlSeconds, key)
