...
public abstract class UnfilteredRowIterators
{
    ...
    public static void digest(ReadCommand command, UnfilteredRowIterator iterator, MessageDigest digest, int version)
    {
        ...
        if (!iterator.isEmpty())
        {
            ...
            // into account if there is no static row or we'd have a digest mismatch between depending on whether the
            // digest is computed on an iterator that has been serialized or not (see CASSANDRA-12090)
